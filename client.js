...
(function(){
    ...
    roulette.login(function(accountName){
        ...
        (async function updateSpin(spin){
            ...
            if(spin && now < spin.maxbettime){
                ...
                window.spin = await roulette.getSpin(now + 30);
                console.log('got spin', window.spin);
            }
            setTimeout(function(){updateSpin(window.spin);}, 1000);
        })();
    });

    // Get selected numbers from a mouse event on the layout.
    window.getCoverage = function(mouseEvent){
    ...
    // Place a bet.
    window.processBet = async function(coverage, larimers){
        ...
        try{
            ...
            if(hash){
                ...
                setTimeout(function(){window.processBet(mouseEvent, larimers);}, 1000);
            }
        }catch(e){
            console.error(e);
            return e;
